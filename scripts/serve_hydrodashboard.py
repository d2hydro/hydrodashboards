...
def start_applications(config):
    ...
    for port in config.ports:
        ...
        write_pid_file(Path(config.log_dir) / f"bokeh_{port}.pid", pid=process.pid)
...
def restart_applications(config, rebuild_time_series=True):
    ...
    terminate_bokeh(ports=config.ports)

    # rebuild cache
    logger = import_logger(log_file="build_cache.log")
    data_model = data.Data(logger=logger, now=datetime.now(), config=config)
...
def main():
    ...
    if is_bokeh_app(app_path):
        ...
        pid_file = Path(config.log_dir) / "hydrodashboards.pid"
        write(pid_file)

        atexit.register(lambda: terminate_bokeh(ports=config.ports))
        restart_applications(config, rebuild_time_series=False)

        # schedule reboot
        restart_time = config.cache_rebuild_time.strftime("%H:%M")
        schedule.every().day.at(restart_time).do(restart_applications, config=config)
