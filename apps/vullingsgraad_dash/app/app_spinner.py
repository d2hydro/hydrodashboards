    ...
    [   
        ...
        html.Div(
            [
                ...
                dcc.Loading(
                    id="loading-combined",
                    type="default",   # 'default' | 'circle' | 'cube' | 'dot' | 'graph'
                    children=[
                        dcc.Graph(
                            id="combined-graph",
                            figure=EMPTY_FIG,  # <- start volledig leeg, assen uit
...
# (optioneel) teller voor aantal builds per peilgebied (alleen logging)
build_counts = {}
...
def update_stylemap(idx, sel, var):
    try:
        ...
        print(f"[ERROR] update_stylemap: {e}", file=sys.stderr, flush=True)
        raise PreventUpdate
...
# ========= Grote grafiek bouwen -> schrijft ALLEEN de fig naar Store =========
@app.callback(
...
def build_combined_figure(sel, var):
    ...
    build_counts.setdefault(sel, 0)
    build_counts[sel] += 1
    print(f"[BUILD] sel={sel} run#{build_counts[sel]}", flush=True)
    ...
    if fig is None:
        ...
        fig.add_trace(
            go.Scatter(x=df_vg.datetime, y=df_vg.vullingsgraad,
        ...
        for y0, y1, colc in [
            ...
            fig.add_shape(type="rect", xref="x1", yref="y1",
                          ...
                          y0=y0, y1=y1, fillcolor=colc, line_width=0,
                          layer="below", row=1, col=1)
        fig.add_trace(
            go.Scatter(x=df_vul.datetime, y=df_vul.vulling,
        ...
        for mid, grp in tb_all.groupby("location_id"):
            ...
            fig.add_trace(
                go.Scatter(x=grp.datetime, y=grp.value/1000, mode="lines",
                           line=dict(color="grey", width=1), opacity=0.7,
                           hovertemplate="datum: %{x|%Y-%m-%d %H:%M}<br>"
                                         "waarde: %{y:.3f}<br>naam: "+naam+"<extra></extra>",
                           showlegend=False),
                row=3, col=1,
            )
        if not df_pgb.empty:
            fig.add_trace(
                go.Scatter(x=df_pgb.datetime, y=df_pgb.waarde/1000, mode="lines",
                           line=dict(color="royalblue", width=3),
                           hovertemplate="datum: %{x|%Y-%m-%d %H:%M}<br>"
                                         "waarde: %{y:.3f}<br>naam: Peilgebied<extra></extra>",
                           showlegend=False),
                row=3, col=1,
            )
        streefpeil = None
        ...
        if streefpeil is not None and pd.notnull(streefpeil):
            x_vals = list(df_pgb["datetime"]) if not df_pgb.empty else sorted(tb_all["datetime"].unique())
            if x_vals:
                y_val = streefpeil / 1000
                fig.add_trace(
                    go.Scatter(x=x_vals, y=[y_val]*len(x_vals), mode="lines",
                ...
                fig.add_annotation(x=x_vals[0], y=y_val, xref="x3", yref="y3",
        ...
        x_min, x_max = df_vg.datetime.min(), df_vg.datetime.max()
        for r in [1, 2, 3]:
            fig.update_xaxes(range=[x_min, x_max], row=r, col=1, automargin=True)
        cache.set(ckey, fig)
        print(f"CACHE MISS combined for {sel}", flush=True)
    else:
        print(f"CACHE HIT combined for {sel}", flush=True)

    payload = {
        "fig": fig.to_dict(),   # de echte figure
        "_nonce": time.time(),  # forceer een change, ook bij cache hit
    }
    return payload


# ========= Vlines toevoegen (los van spinner) =========
@app.callback(
...
def add_vline_to_combined(fig_payload, idx):
    if fig_payload is None or idx is None:
        raise PreventUpdate
    # Haal de echte fig eruit
    base = fig_payload.get("fig")
    if base is None:
        raise PreventUpdate
...
if __name__ == "__main__":
    app.run(debug=True, use_reloader=False)  # voorkom dubbele processen
