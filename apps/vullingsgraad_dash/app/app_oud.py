...
# === Voeg caching toe (direct na dash.Dash) ===
...
app.layout = html.Div([
    ...
    dl.Map(center=[(lat_sw+lat_ne)/2,(lon_sw+lon_ne)/2], bounds=leaflet_bounds, style={"height":"100vh","width":"100%"}, preferCanvas=True, children=[
        dl.TileLayer(), dl.LayerGroup(id="marker-mpn"),
    ...
    html.Div([dcc.Graph(id="combined-graph", config={"displayModeBar":True,"scrollZoom":True}, style={"height":"100%","minHeight":0})], style=def_layout),

    html.Div([dcc.Store(id="is-playing", data=False), html.Button(id="playpause-button", n_clicks=0, style={"width":"72px"}), html.Div(initial_label, id="datum-label", style={"fontWeight":"bold"}),
             html.Div([dcc.Graph(id="mini-tijdserie", config={"displayModeBar":False}, style={"height":"50px","width":"350px","position":"absolute","top":0,"left":"25px","pointerEvents":"none"}),
                       dcc.Slider(id="tijdslider", min=0, max=len(all_datetimes)-1, value=default_index, updatemode="mouseup")], style={"position":"relative","width":"400px","height":"50px","display":"inline-block"})],
    ...
    dcc.Interval(id="interval", interval=1000, disabled=True), html.Div(id="click-output")
])
...
@app.callback(
    [Output("combined-graph","figure"), Output("mini-tijdserie","figure")],
...
def update_all(sel, var, idx):
    ...
    # Combined uit cache
    ckey=f"combined_{sel}"; fig=cache.get(ckey)
    if fig is None:
        # vullingsgraad & vulling data
        tb=ds_vg.to_table(filter=(ds.field('location_id')==sel), columns=["datetime","value"]); df_vg=pd.DataFrame({"datetime":pd.to_datetime(tb['datetime'].to_pylist()),"vullingsgraad":tb['value'].to_pylist()})
        tb=ds_vul.to_table(filter=(ds.field('location_id')==sel), columns=["datetime","value"]); df_vul=pd.DataFrame({"datetime":pd.to_datetime(tb['datetime'].to_pylist()),"vulling":tb['value'].to_pylist()})
        tb=ds_wlvl_pgb.to_table(filter=(ds.field('location_id')==sel), columns=["datetime","value"]); df_pgb=pd.DataFrame({"datetime":pd.to_datetime(tb['datetime'].to_pylist()),"waarde":tb['value'].to_pylist()})
        # meetpunt data
        ...
        fig=make_subplots(rows=3,cols=1,shared_xaxes=True,vertical_spacing=0.06,subplot_titles=["Vullingsgraad [%]","Vulling [mm]","Waterstand [mNAP]"])
        # tekens
        ...
        for y0,y1,colc in [(0,25,'rgba(120,200,120,0.4)'),(25,50,'rgba(255,255,100,0.4)'),(50,75,'rgba(255,200,100,0.4)'),(75,100,'rgba(255,100,100,0.4)')]: fig.add_shape(type='rect',xref='x1',yref='y1', x0=df_vg.datetime.min(),x1=df_vg.datetime.max(),y0=y0,y1=y1,fillcolor=colc,line_width=0,layer='below',row=1,col=1)
        ...
        for y0,y1,colc in [(0,10,'#ffffff'),(10,20,'#b4d3e7'),(20,30,'#72b2d7'),(30,40,'#3e91c4'),(40,60,'#1c5fa5')]: fig.add_shape(type='rect',xref='x2',yref='y2', x0=df_vul.datetime.min(),x1=df_vul.datetime.max(),y0=y0,y1=y1,fillcolor=colc,line_width=0,layer='below',row=2,col=1)
        for mid,grp in tb_all.groupby('location_id'): naam=df_locs_mpn.loc[df_locs_mpn.location_id==mid,'naam'].iat[0]; fig.add_trace(go.Scattergl(x=grp.datetime,y=grp.value/1000,mode='lines',line=dict(color='grey',width=1),opacity=0.7,hovertemplate='datum: %{x|%Y-%m-%d %H:%M}<br>waarde: %{y:.3f}<br>naam: '+naam+'<extra></extra>',showlegend=False),row=3,col=1)
        if not df_pgb.empty: fig.add_trace(go.Scattergl(x=df_pgb.datetime,y=df_pgb.waarde/1000,mode='lines',line=dict(color='royalblue',width=3),hovertemplate='datum: %{x|%Y-%m-%d %H:%M}<br>waarde: %{y:.3f}<br>naam: Peilgebied<extra></extra>',showlegend=False),row=3,col=1)
        fig.update_yaxes(range=[0,100],fixedrange=True,row=1,col=1); fig.update_yaxes(range=[0,60],fixedrange=True,row=2,col=1); fig.update_yaxes(fixedrange=True,row=3,col=1); fig.update_xaxes(title_text='Tijd',row=3,col=1)
        fig.update_layout(height=900,margin=dict(l=40,r=10,t=60,b=40),plot_bgcolor='white',hovermode='x unified',font=dict(size=13),showlegend=False)
        cache.set(ckey,fig); print(f"CACHE MISS combined for {sel}")
    else:
        print(f"CACHE HIT combined for {sel}")
    # mini
    mkey=f"mini_{sel}_{var}"; mini=cache.get(mkey)
    if mini is None:
        ds_sel = ds_vg if var=='vullingsgraad' else ds_vul
        # correcte code
        tb = ds_sel.to_table(
            filter=(ds.field('location_id') == sel),
            columns=['datetime','value']
        )
        series = pd.Series(
            tb['value'].to_pylist(),
            index=pd.to_datetime(tb['datetime'].to_pylist())
        )

        vals=[series.get(pd.Timestamp(dt),None) for dt in all_datetimes]; idx0=int(idx)
        mini=go.Figure(go.Scatter(x=list(range(len(all_datetimes))),y=vals,mode='lines',line=dict(width=2),hoverinfo='skip',showlegend=False)); mini.add_vline(x=idx0,line_width=2,line_dash='dash'); mini.update_layout(margin=dict(l=0,r=0,t=0,b=0),height=50,plot_bgcolor='rgba(0,0,0,0)',xaxis=dict(visible=False,range=[0,len(all_datetimes)-1],fixedrange=True),yaxis=dict(visible=False,fixedrange=True))
        cache.set(mkey,mini); print(f"CACHE MISS mini for {mkey}")
    else:
