                        ...
                        dl.GeoJSON(
                            ...
                            options={
                                ...
                                "onEachFeature": assign(
                                    "function(f, layer){ if(layer?.bringToFront){ layer.bringToFront(); } }"
                                ),
                                "pointToLayer": assign(
                                    """
                    ...
                    [
                        html.Label(
                            [
                                ...
                                html.Span(
                                    "?",
                                    title="Variabele voor kaartkleuren",
                                    style={"cursor": "help"},
                                ),
                            ],
                            style={"fontSize": "13px"},
                        ...
                        html.Label(
                            [
                                ...
                                html.Span(
                                    "?",
                                    title="Peilgebied voor grafieken",
                                    style={"cursor": "help"},
                                ),
                            ],
                            style={"fontSize": "13px"},
...
def build_combined_figure(sel, _var):
    ...
    df_vg = time_series_cache.get_time_series(
        "VullingsgraadOutput", "vullingsgraad", location_ids=[sel]
    )
    df_vul = time_series_cache.get_time_series(
        "VullingsgraadOutput", "vulling_mm", location_ids=[sel]
    )
    df_pgb = time_series_cache.get_time_series(
        "PeilgebiedWaterstandOutput", "H.meting", location_ids=[sel]
    )
    ...
    for xref in ["x1", "x2", "x3"]:
        cursor_shapes.append(
            go.layout.Shape(
                ...
                xref=xref,
                yref="paper",
                line=dict(color=halo_col, width=6),
                layer="above",
            )
        )
    for xref in ["x1", "x2", "x3"]:
        cursor_shapes.append(
            go.layout.Shape(
                ...
                xref=xref,
                yref="paper",
                line=dict(color=core_col, width=2),
                layer="above",
            )
        )
...
@app.callback(
    ...
    ],  # blijft Input om re-style te forceren
)
def update_mpn_markers(
    selected_location_id, idx, mpn_clickdata, clicked_trace_id, _relayoutData
):
    """
    Houdt bij:
    ...
    if clicked_trace_id:
        ...
        sel_id = mpn_clickdata["properties"].get("location_id") or mpn_clickdata[
            "properties"
        ].get("id")
